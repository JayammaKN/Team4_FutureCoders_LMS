...
Locator: getByText('happytwo', { exact: true })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('happytwo', { exact: true })
...
- grid:
  ...
  - rowgroup:
    - row "happytwoone Nova Description Edited Active":
      - gridcell:
        - checkbox
        - checkbox
      - gridcell "happytwoone"
...
- button "" [disabled]
- button "" [disabled]
- text: In total there are 54 programs.
```
...
> 85  |     await expect(this.page.getByText(this.programName, { exact: true })).toBeVisible();
  ...
  87  |     await this.page.waitForTimeout(500)
  88  |   }
  89  | 
  90  |   async editProgram() {
  91  |     await this.editButton.click();
  92  | 
  93  |   }
  94  | 
  95  |   async editProgramDetails() {
  96  |     await this.programNameField.fill(this.newProgramName);
  97  |     await this.programDescriptionField.fill(this.newProgramDescription);
  98  |     await this.programSaveButton.click();
  99  |     await expect(this.newSuccessMessage).toBeVisible();
  100 | 
  101 |   }
  102 | 
  103 | }
