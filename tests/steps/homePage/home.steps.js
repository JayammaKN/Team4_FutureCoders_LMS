...
Given("Admin is on the home page", async ({ homeFixture }) => {
  await homeFixture.navigateToHome();
});

Then(
  "Admin should see LMS - Learning management system as title",
  async ({ homeFixture }) => {const actualTitle = await homeFixture.getTitleText();

    await expect(actualTitle).toBe(homeData.title);
  },
);
...
Then(
  ...
  async ({ homeFixture }) => {
    const isTopLeft = await homeFixture.isTitleTopLeft();

    await expect(isTopLeft).toBe(true);
  },
);
...
Then(
  ...
  async ({ homeFixture }) => {
    const actualNavTexts = await homeFixture.getNavTexts();

    await expect(actualNavTexts).toEqual(homeData.navLinks);
  },
);
...
Then(
  ...
  async ({ homeFixture }) => {
    const actualTitle = await homeFixture.getTitleText();
...
Then(
  ...
  async ({ homeFixture }) => {
    const isTopRight = await homeFixture.isNavTopRight();

    await expect(isTopRight).toBe(true);
  },
);
...
Then(
  ...
  async ({ homeFixture, envFixture }) => {
    const welcomeText = await homeFixture.getWelcomeText();

    await expect(welcomeText).toContain(homeData.welcome);

    await expect(welcomeText).toContain(envFixture.username);

    await expect(welcomeText.toLowerCase()).toContain(
...
Then(
  ...
  async ({ homeFixture }) => {
    ...
    expect(chartData.labels).toContain("Active");

    expect(chartData.active).toBeGreaterThanOrEqual(0);

    expect(chartData.inactive).toBeGreaterThanOrEqual(0);
  },
);
...
Then(
  ...
  async ({ homeFixture }) => {
    const actualHeaders = await homeFixture.getStaffHeaders();

    await expect(actualHeaders).toEqual(homeData.staffHeaders);
  },
);
